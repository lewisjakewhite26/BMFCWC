      ...
      {fixtures.length === 0 ? (
        ...
        fixtures.map((fixture) => (
          <MatchCard
            ...
            fixture={fixture}
            prediction={predictions.get(fixture.id)}
            locked={predictionsLocked || gameDay.status !== 'open' || !isCurrent}
            onSave={gameDay.status === 'open' && isCurrent && !predictionsLocked ? onSave : undefined}
            onConfirmChange={isCurrent ? onConfirmChange : undefined}
          />
        ))
      )}
    </div>
